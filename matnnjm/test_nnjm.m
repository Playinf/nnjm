...
options.source_context = 11;
options.target_context = 3;
...
options.control = 0.1;
options.source_vocabulary = 'sample/source.vocab';
options.target_vocabulary = 'sample/target.vocab';
options.output_vocabulary = 'sample/output.vocab';
%model = nnjm(options);
%model = nnjm_init(model, options);
...
for i = start:epoch
  % training data
  file = fopen('sample/sample.numberized', 'r');
  data = zeros(batch, order);
  cost = 0;
  count = 0;
  model_name = [model_basename, num2str(i)];
  model.options.output = [model_name, '.nnjm'];
  ...
  while ~feof(file)
    ...
    if n ~= 0
      ...
      data = data + 1;
      t1 = clock;
      [model, c] = nnjm_train(model, data, options);
      t2 = clock;
      time = etime(t2, t1);
      cost = cost + c;
    end
