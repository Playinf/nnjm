function save_model(model)
  ...
  % write options
  fprintf(file, '24\n');
  fprintf(file, 'model: nnjm\n');
  fprintf(file, 'order: %d\n', model.context + 1);
  fprintf(file, 'source-context: %d\n', model.source_context);
  fprintf(file, 'target-context: %d\n', model.target_context);
  ...
  % write vocabulary
  write_vocab(file, model.source_vocab);
  write_vocab(file, model.target_vocab);
